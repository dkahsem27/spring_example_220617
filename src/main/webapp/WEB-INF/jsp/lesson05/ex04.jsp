...
<body>
	...
	<h1>2. 특정 문자열이 있는지 확인</h1> <%-- true/false --%>
	...
	<h1>3. 특정 문자열로 시작하는지 확인</h1> <%-- true/false --%>
	No로 시작하는가? ${fn:startsWith(str1, 'No')}<br>
	
	<h1>4. 특정 문자열로 끝나는지 확인</h1>
	n으로 끝나는가? ${fn:endsWith(str1, 'n')}<br> <%-- false --%>
	.으로 끝나는가?(문자 하나로는 검사가 안 됨) ${fn:endsWith(str1, '.')}<br> <%-- false --%>
	gain.으로 끝나는가?(단어채로 검사는 됨) ${fn:endsWith(str1, 'gain.')}<br> <%-- true --%>
